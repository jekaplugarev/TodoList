...
export type FetchTasksActionType = ReturnType<typeof fetchTaskAC>

export const fetchTasksThunkCreator = (todoId: string) => (dispatch: Dispatch) => {
    todolistsAPI.getTasks(todoId)
        .then((res) => {
            const tasks = res.data.items
            dispatch(fetchTaskAC(todoId, tasks))
        })
}
...
export const updateTaskStatusTC = (todoId: string, taskId: string, status: TaskStatuses) => (dispatch: Dispatch, getState: () => AppRootStateType) => {
    ...
    const allTaskForClickedTodo = allTasks[todoId]
    const cliackedTask = allTaskForClickedTodo.find((t) => {
        return t.id === taskId
    })
    if (cliackedTask) {
        const model: UpdateTaskModelType = {
            title: cliackedTask.title,
            status: status,
            description: cliackedTask.description,
            startDate: cliackedTask.startDate,
            priority: cliackedTask.priority,
            deadline: cliackedTask.deadline
        }
